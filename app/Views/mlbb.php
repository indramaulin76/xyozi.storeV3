...
<div class="flex flex-col gap-3 w-full max-w-screen-xl">
  <div class="w-full max-w-screen-xl">
    <img class="object-cover w-full" src="<?= base_url(); ?>img/banner/20240322MLBB-banner.jpg" alt="class" loading="lazy">
  </div>
    ...
    <div class="flex flex-col gap-3 relative w-full">
      <div class="sticky-content">
        <div class="flex items-center gap-3 bg-murky-800 border-y border-murky-600 top-content shadow-2xl p-4">
            <img class="object-cover games-img lazy-image" data-src="<?= base_url(); ?>img/banner/20240322MLBB-banner.jpg" alt="MLBB" loading="lazy">
            <div class="flex flex-col">
                <h5 class="text-xl italic font-medium text-gray-900 dark:text-white">Mobile Legends</h5>       
                <p class="text-sm text-gray-300">MLBB</p>
    ...
    <div class="flex flex-col gap-3 w-full px-3 py-2">
      <div class="w-full rounded-lg shadow bg-murky-800" id="dataAkun">
        <div class="flex border-b border-gray-200 dark:border-gray-700">
          ...
          <h3 class="flex w-full items-center justify-between px-2 py-2 text-gray-900 dark:text-white font-semibold leading-6 sm:px-4">Masukkan Nama Kamu</h3>
        </div>
        <div class="space-y-3 p-4">
          <input type="text" name="nama" id="nama" class="custom-input block w-full mb-2" value="" placeholder="Masukan Nama Kamu" required="">
      ...
      <div class="w-full rounded-lg shadow bg-murky-800" id="dataNominal">
        <div class="flex border-b border-gray-200 dark:border-gray-700">
          ...
          <h3 class="flex w-full items-center justify-between px-2 py-2 text-gray-900 dark:text-white font-semibold leading-6 sm:px-4">Pilih Produk</h3>
        </div>
        <div class="space-y-3 p-4">
            <?php if (!empty($produk)) : ?>
                <div class="grid grid-cols-2 gap-3 pb-3 pt-3">
                    ...
                    if (!empty($produk)) {
                        foreach ($produk as $p) {
                            $Logo = "mobile-legends-bang-bang-logo-71C2E06F9D6585675.png";
                            ?>
                            <input type="hidden" name="target" id="target" value="<?= $p['durasi'] ?>">
                            <input type="hidden" name="tipe" id="tipe" value="<?= $p['tipe'] ?>">
                            <input type="hidden" name="provider" id="provider" value="<?= $p['provider'] ?>">
                            <div class="flex flex-row gap-3">
                                <button onclick="selectProduct('<?= $p['durasi'] ?>', '<?= $p['harga'] ?>', '<?= $p['layanan_id'] ?>')" id="productBtn<?= $p['durasi'] ?>" class="flex flex-row items-center justify-between rounded-lg p-2 false bg-gray-300 w-full relative group border-2 border-transparent">
                                    <div class="w-full flex justify-between items-center">
                                        <div class="flex flex-col flex-1">
                                            <div class="flex flex-col false">
                                                <span class="text-xs font-semibold text-left line-clamp-4 text-gray-900 dark:text-gray-900"><?= $p['durasi'] ?> <?= $p['tipe'] ?></span>
                                            </div>
                    ...
                    <button id="metodeBtn<?= $m['kode'] ?>" class="relative bg-gray-300 border-2 border-transparent gap-2 flex w-full flex-col rounded-lg p-3">
                        <div class="flex flex-row gap-2 justify-between w-full">
                            <div class="flex items-center space-x-3">
                                <img src="<?= base_url(); ?>img/metode/<?= $m['gambar'] ?>" alt="<?= $m['nama'] ?>" class="flex flex-col w-8 max-h-4" loading="lazy">
                    ...
                    <?php if ($m['kategori'] == 'ewallet'): ?>
                      <div class="pb-2">
                        <button id="metodeBtn<?= $m['kode'] ?>" class="p-3 relative bg-gray-300 border-2 border-transparent gap-2 flex w-full flex-col rounded-lg">
                          <img src="<?= base_url(); ?>img/metode/<?= $m['gambar'] ?>" alt="<?= $games['nama'] ?>" class="flex flex-col w-16 h-7" loading="lazy">
              ...
              <div class="flex gap-3 p-3 bg-murky-300 rounded-b-lg">
                <?php foreach ($metode as $m) : ?>
                    ...
                    <div class="overflow-hidden overflow-x-scroll">
                      <img src="<?= base_url(); ?>img/metode/<?= $m['gambar'] ?>" alt="<?= $games['nama'] ?>" class="flex flex-col w-10 h-4" loading="lazy">
                    </div>
                    <?php endif; ?>
                <?php endforeach; ?>
              </div>
              ...
              <div class="flex gap-3 p-3 bg-murky-300 rounded-b-lg">
                ...
                <?php foreach ($metode as $m) : ?>
                    ...
                    <div class="flex-shrink-0 mr-4">
                      <img src="<?= base_url(); ?>img/metode/<?= $m['gambar'] ?>" alt="<?= $games['nama'] ?>" class="mr-2 w-10 h-4" loading="lazy">
                    </div>
                    <?php endif; ?>
                <?php endforeach; ?>
                </div>
...
<script>
    ...
    function selectProduct(kodeProduk, hargaJual, namaProduk) {
        <?php foreach ($produk as $p) : ?>
          document.getElementById('productBtn<?= $p['durasi'] ?>').style.border = '2px solid transparent';
          document.getElementById('productBtn<?= $p['durasi'] ?>').classList.remove('bg-white');
          document.getElementById('productBtn<?= $p['durasi'] ?>').classList.add('bg-gray-300');
          document.getElementById('selectedIndicator<?= $p['durasi'] ?>').classList.add('hidden');
        <?php endforeach; ?>
    ...
    function placeOrder() {
        ...
        if (!nama) {
          notyf.error('Harap masukkan nama kamu dahulu');
      ...
      Swal.fire({
        ...
        html: `
          <table class="w-full text-left text-base">
            <tr>
              <th class="whitespace-nowrap pr-4">NAMA</th>
              <td>: ${nama}</td>
            </tr>
            <tr>
                <th class="whitespace-nowrap pr-4">Produk</th>
                <td>: ${selectedProductCode} ${tipe}</td>
            </tr>
            <tr>
